...
int findCode(const string name, const string table[])
{
  int index {0};
...
void processIF(Statement &statement, VariableSet &variableSet)
{
  ...
  string opd;
  cin >> opd;
  statement.opd1 = findVar(opd, variableSet);
  string opr;
  cin >> opr;
  statement.compopr = findOprCode(opr);
  cin >> opd;
  statement.opd2 = findVar(opd, variableSet);
  cin >> gotoStr;
  assert(gotoStr == "GOTO");
...
void processASSIGN(Statement &statement, const string assigned, 
		   VariableSet &variableSet)
{
  statement.target = findVar(assigned, variableSet);
  string assign;
  cin >> assign;
  assert(assign == "=");
  string opd;
		 ...
		 Statement statements[])
{
  ...
  while (cin >> first) {
    ...
    case PRINT:
      ...
      cin >> opd;
      statement.target = findVar(opd, variableSet);
      break;
...
int arithemetic(const int operand1, const int operand2, const int opr)
{
  switch (opr) {
...
void readVariable(VariableSet &variableSet)
{
  ...
  cin >> name;
  while (name != "END") {
    Variable &variable = variableSet.variables[variableSet.count];
    variable.name = name;
    string assignStr;
    cin >> assignStr;
    assert(assignStr == "=");
