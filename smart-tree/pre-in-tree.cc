...
//--buildTree--
NodePtr PreInTree::buildTree(vector <int> pre, vector <int> in)
{
  if (pre.size() == 0)
    return nullptr;
  int root = pre[0];
  ...
  return
    (new TreeNode(root,
		  buildTree(vector <int>
			    (pre.begin() + 1, pre.begin() + 1 + leftSize),
			    vector <int>
			    (in.begin(), in.begin() + leftSize)),
		  buildTree(vector <int>
			    (pre.begin() + leftSize + 1, pre.end()),
			    vector <int>
			    (in.begin() + leftSize + 1, in.end()))));
}
//--end--
